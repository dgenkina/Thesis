...
def getChernTB(omega, delta, epsilon, tx, S, m0,c=4.0/3.0,kList=np.linspace(-1.0,1.0,600),Flat = True,save=False,plots=True):

    kList, E, m, pops = tbh.plotSynDimBandStructGenTBClean(omega, delta, epsilon, tx, S, m0,c=c,kList=kList,Flat=Flat,save=save,plots=plots)
    ...
    xMth,dxMth=getMaxFitTheory(kList,pops[:,0,S+1],np.ones(pops[:,0].size)*0.001,indC=indM,plot=plots)  #[np.where(pops[:,0,S-1]==np.max(pops[:,0,S-1]))]
    x0th,dx0th=getMaxFitTheory(kList,pops[:,0,S],np.ones(pops[:,0].size)*0.001,indC=ind0,plot=plots)
    xPth,dxPth=getMaxFitTheory(kList,pops[:,0,S-1],np.ones(pops[:,0].size)*0.001,indC=indP,plot=plots)
...
delChernList1=np.zeros(omegaList.size)
delChernList2=np.zeros(omegaList.size)
for ind, omega in enumerate(omegaList):
    pops, chernList1[ind],delChernList1[ind]=getChernTB(omega, 0.0, 0.0, 0.5,1,0,c=c,Flat=Flat, plots=False)
    pops, chernList2[ind],delChernList2[ind]=getChernTB(omega, 0.0, 0.0, 0.5,2,0,c=c,Flat=Flat, plots=False)
...
np.savez(filename, c=c,Flat=Flat, omegaList = omegaList, chernList1=chernList1,chernList2=chernList2, 
         delChernList1=delChernList1,delChernList2=delChernList2,epsilon=0.0,
         tx = 0.5, m0=0,kList=np.linspace(-1.0,1.0,200))

#q=3.0
#c=4.0/q#1064.0/790.0#0.0#
#Flat=True
#Slist=np.arange(1,35)
#chernList=np.zeros(Slist.size)
#delChernList=np.zeros(Slist.size)
#for ind, Sloc in enumerate(Slist):
#    pops, chernList[ind],delChernList[ind]=getChernTB(0.5, 0.0, 0.0, 0.5,Sloc,0,c=4.0/3.0,Flat=True, plots=False)
#    print Sloc
#    
#    
#figure3=plt.figure()
#pan3=figure3.add_subplot(111)
#pan3.errorbar(Slist,chernList,yerr=delChernList,fmt= 'bo')
#pan3.set_xlabel(r'F')
#pan3.set_ylabel(r'Chern number')
#
#filename = 'ChernOfSystemSizeOmega05'
#np.savez(filename, c=c,Flat=Flat, Slist = Slist, chernList=chernList,delChernList=delChernList, omega=0.5,delta=0.0,epsilon=0.0,tx=0.5,m0=0,kList=np.linspace(-1.0,1.0,200))