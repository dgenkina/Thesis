...
def Image(Inot, tfinal,steps,ODinit):         
 ...
 #   Inot = 1.3
    ...
    od1 = od0 + (Inot-Ifinaltot)
    od2 = od1 - ((B*(trange+dt))**2.0)*(1.0/(Ifinaltot+1.0)-1.0/(Inot+1.0)+np.log((Ifinaltot+1.0)/(Inot+1.0)))/3.0
    
    return (od, od0, od1, od2, Ifinaltot, [atoms[i].vlist for i in range(superAtomNumber)], dz)
