...
def Image(Inot, tfinal,steps,width): #ODinit):         
    ...
    def n(x):
        return np.exp(-((x-zfinal/2.0)**2)/(2.0*width**2))#ODinit/zfinal/A
    ...
    superSize = 1e11 #number of atoms per unit area in one superatom
 ...
 #   Inot = 1.3
    ...
    return (od, od0, od1, Ifinaltot, [atoms[i].vlist for i in atomIndexes], gaussian,[atoms[i].zlist for i in atomIndexes],superAtomNumber)
