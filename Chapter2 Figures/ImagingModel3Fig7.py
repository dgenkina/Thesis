...
def ImageS(Inot, tfinal,steps,ODinit):         
 ...
 #   Inot = 1.3
    ...
    nu2 = od - (Inot-Ifinaltot) + ((B*(trange+dt))**2.0)*(1.0/(Ifinaltot+1.0)-1.0/(Inot+1.0)+np.log((Ifinaltot+1.0)/(Inot+1.0)))/3.0
    
    return (od, nu, nu1, nu2, Ifinaltot, [atoms[i].vlist for i in range(superAtomNumber)], dz)

def ImageM(Inot, tfinal,steps,ODinit):         
    """define initial atom distribution, and convert to superatoms.
    Assign initial position and velocity to each superatom"""
    zfinal = 0.0001 #m, so ~100um
    def n(x):
