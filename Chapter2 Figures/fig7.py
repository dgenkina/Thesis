...
nupanel.plot(I0range, [nu1[i,tind] for i in range(I0range.size)], 'k-', linewidth=2, label=r'$OD_1$')
nupanel.plot(I0range, [nu2[i,tind] for i in range(I0range.size)], 'b-', linewidth=2, label=r'$OD_2$')
nupanel.plot(I0range, [nuS[i,tind] for i in range(I0range.size)], 'g-', linewidth=2, label=r'$OD_{corr1}$')
nupanel.plot(I0range, [nuM[i,tind] for i in range(I0range.size)], 'go', ms=3, label=r'$OD_{corr2}$')
nupanel.axhline(y=0,linestyle='--',color='k')
nupanel.set_ylabel('Optical Depth')
nupanel.set_xticks([])

nupanel.text(3.5,1.1,r'Eq. 2.4', size=10)
nupanel.text(3.5,-2.5,r'Eq. 2.10', size=10)
nupanel.text(3.5,0.28,r'$OD^{\rm{sim}}$',  size=10)
