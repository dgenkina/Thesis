# -*- coding: utf-8 -*-
"""
Created on Wed Oct 25 12:38:05 2017
...
def plotLatticeBandStruct(U, n,kList=np.linspace(-1.0,1.0,600),save=False,plot=True):
    i=0  
        

    N=2*n+1
    E=np.zeros((kList.size,N))

    pops=np.zeros((kList.size,N))
    ...
    if plot:
      ...
      #  panel.set_title(r'$\Omega$ = '+str(np.round(omega,2))+r'$E_L$, $\delta$ = '+str(np.round(delta,3))+r'$E_L$, $\epsilon$ = '+str(np.round(epsilon,3))+r'$E_L$, U = '+str(np.round(U,2))+r'$E_L$')
        for i in range(3):   
            d=panel.scatter(kList,E[:,i],c='b', marker='o',linewidths=0.0)
        panel.set_xlabel(r'Crystal momentum [$k_L$]')
...
def plotPulseTrainOfQ(qlist,pulseTime,Npulses,U):
    ...
    figure=plt.figure()
    panel=figure.add_subplot(1,1,1)
    panel.set_title('N,U,pulseTime = %.0f, %.2f, %.2f' %(Npulses,U,pulseTime*1e6))
...
#filestart=34
#filestop=63
#fileroot = 'Y:/Data/2017/November/01/PIXIS_01Nov2017' 
...
#datafile=np.load('09Nov2017_files_728-757.npz')
#time=datafile['tlist']
...
#Uguess=1.5
#kguess=0.0
#popt,pcov = optimize.curve_fit(propagateLatHamFit,tRecoils,fractions.flatten(), p0=(Uguess,kguess))
#print popt,pcov
...
#figure=plt.figure()
#panel=figure.add_subplot(1,1,1)
#panel.set_title('U = %.2f +/- %.3f E_L, k = %.2f +/- %.3f ' %(popt[0],np.sqrt(np.diag(pcov)[0]),popt[1],np.sqrt(np.diag(pcov)[1])))#' %(popt[0],np.sqrt(np.diag(pcov)[0])))#, epsilon = ' + str(np.round(popt[2],3))+ ' Er')
