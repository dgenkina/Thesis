...
def rampedOnPsi(k,omega,delta,epsilon,U,n,rampOnt=0.0003*Erecoil/hbar):
    ...
    dt=tlist[1]-tlist[0]
    Energy1, V1 = LA.eig(RamanLatHamiltonian(0.0, 0.0, 0.0, epsilon, U, n))
...
dataFile=np.load('26Aug2016_files_459-522.npz')
...
maxInd=np.int(fractionP.size*2/3)
...
#a=np.array(tRecoils)
#b=np.array(frac0)
popt,pcov = optimize.curve_fit(HofT,tRecoils[:maxInd],fractions.flatten(), p0=(0.61,0.001,5.0))
